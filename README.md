...
After changing the Press system version, run `node scripts/sync-runtime-cache-keys.js --write` to synchronize static runtime cache keys from `assets/press-system.json`. CI runs the same script with `--check` and blocks releases when runtime cache keys are stale.
