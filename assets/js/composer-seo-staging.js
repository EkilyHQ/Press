...
import { PUBLIC_CONTENT_SECURITY_POLICY } from './content-security-policy.mjs';
...
function buildDefaultIndexHtml(metaBlock, lang) {
  ...
  html += `  <meta http-equiv="Content-Security-Policy" content="${PUBLIC_CONTENT_SECURITY_POLICY}">\n\n`;
  html += metaSection;
...
export function createSeoStagingProvider({
  ...
  getIndexVariantLocation = (value) =>
    typeof value === 'string' ? value : String((value && (value.location || value.path)) || ''),
  fetchImpl = null,
  getLocationOrigin = () => '',
  getDocumentLang = () => '',
  consoleRef = null
    ...
    keys.forEach((key) => {
      ...
      Object.keys(entry).forEach((lang) => {
        ...
        if (Array.isArray(value)) {
          const normalized = value.map((item) => getIndexVariantLocation(item)).filter((item) => item);
  ...
  async function getCommitFiles() {
    try {
      ...
      const robotsTxt = ensureTrailingNewline(
        generateSeoRobotsTxt(siteState, {
