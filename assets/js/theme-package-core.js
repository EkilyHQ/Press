...
function validateThemeRouteHelperContract(entries, contractVersion) {
  ...
  entries.forEach((entry) => {
    ...
    if (containsForbiddenV4RouteConstruction(source, { path: entry.path, files: routeGuardFiles })) {
      throw new Error(
        `Theme contractVersion 4 requires router href helpers instead of public route construction in ${entry.path}.`
      );
    }
  });
}
...
function normalizeRegistrySource(input, fallbackType) {
  ...
  const type =
    safeString(source.type || fallbackType || 'manual')
      .trim()
      .toLowerCase() || 'manual';
...
export function themeFilesFromManifest(manifest) {
  ...
  add('theme.json');
  const styles =
    manifest && Array.isArray(manifest.styles)
