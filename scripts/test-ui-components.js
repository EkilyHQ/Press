...
assert.match(components, /if \(current && !seen\.has\(current\)\) \{[\s\S]*option\.disabled = true;[\s\S]*option\.hidden = true;[\s\S]*select\.insertBefore\(option, select\.firstChild\);[\s\S]*select\.value = current;[\s\S]*\} else if \(current\) \{[\s\S]*select\.value = current;/, 'press-theme-controls should render a disabled current-language placeholder when the active language is outside the public list');
...
assert.match(theme, /function refreshThemeControlsLanguages\(component\)[\s\S]*component\.setLanguages\(getLanguageOptions\(\), getCurrentLang\(\)\)/, 'theme controls should centralize public language option refresh');
assert.match(theme, /getPublicLanguageOptions\(siteConfig\)/, 'theme controls should source public language options from the i18n public language policy API');
...
assert.match(nativeInteractions, /mount\(\{[\s\S]*themeContext: params\.themeContext \|\| params\.context,[\s\S]*features: params\.features \|\| \(params\.context && params\.context\.features\),[\s\S]*siteConfig: params\.siteConfig \|\| \(params\.context && params\.context\.siteConfig\) \|\| \{\}/, 'native theme controls should forward siteConfig so public language policy reaches press-theme-controls');
