...
assert.match(main, /await ensureThemeLayout\(\);\s*setBootProgress\(0\.6\);/, 'main should advance boot progress after the theme layout mounts');
...
assert.match(editorPreviewRuntime, /themeLayout\.ensureThemeLayout\(\{ pack: requestedPack, persist: false, reset \}\)/, 'editor preview runtime should mount themes without persisting themePack through its injected layout controller');
