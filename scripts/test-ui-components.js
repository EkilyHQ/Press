...
assert.match(editorPreviewRuntime, /export function createEditorPreviewRuntimeController\(previewRuntime = createEditorPreviewAppRuntime\(\)\)[\s\S]*function postToParent\(payload\) \{[\s\S]*previewRuntime\.postToParent\(payload\)[\s\S]*function beginPreviewRender\(payload\) \{[\s\S]*previewRuntime\.beginRender\(payload && payload\.requestId\)[\s\S]*function isCurrentPreviewRender\(requestId\) \{[\s\S]*previewRuntime\.isCurrentRender\(requestId\)[\s\S]*function applyPreviewColorMode\(siteConfig = \{\}\) \{[\s\S]*previewRuntime\.applyColorMode\(siteConfig\)[\s\S]*previewRuntime\.applyThemeStyleLinks\(\{[\s\S]*previewRuntime\.querySelector\('\[data-theme-region="main"\], \.native-mainview'\)[\s\S]*previewRuntime\.getThemeLayoutPackFallback\(\)[\s\S]*previewRuntime\.getBody\(\)[\s\S]*previewRuntime\.getPreviewStatusElement\(\)[\s\S]*function start\(\) \{[\s\S]*previewRuntime\.onRenderMessage\(\(event\) => \{[\s\S]*previewRuntime\.isTrustedMessageEvent\(event\)[\s\S]*createEditorPreviewRuntimeController\(\)\.start\(\);/, 'editor preview runtime should route iframe app effects, render state, and DOM fallbacks through an explicit controller boundary');
