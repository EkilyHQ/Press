...
assert.match(editorMain, /PREVIEW_RENDER_MESSAGE = 'press-editor-preview-render'[\s\S]*editorMainRuntime\.postMessage\(frame\.contentWindow, payload\)/, 'editor preview should send render payloads to the iframe runtime through the editor runtime boundary');
...
assert.match(editorMain, /const previewOpenButton = editorMainRuntime\.getElementById\('btnOpenPreview'\);[\s\S]*previewOpenButton\.addEventListener\('click'[\s\S]*openPreviewOverlay\(\);/, 'standalone editor preview button should open the temporary overlay');
...
assert.match(editorMain, /PREVIEW_OVERLAY_CLOSE_MS = 260[\s\S]*editorMainRuntime\.requestFrame\(\(\) => \{[\s\S]*previewWrap\.classList\.add\('is-open'\)[\s\S]*previewWrap\.classList\.add\('is-closing'\)[\s\S]*editorMainRuntime\.setTimer\(\(\) => \{[\s\S]*previewWrap\.hidden = true;[\s\S]*resetPreviewViewportWidth\(\);[\s\S]*\}, PREVIEW_OVERLAY_CLOSE_MS\)/, 'preview overlay should add open and closing classes before hiding after the close animation through the editor runtime boundary');
...
assert.match(editorBlocks, /syncActiveTableAlignmentFromEditable\(activeBlock, editable \|\| blocksState\.getActiveEditable\(\) \|\| runtime\.getActiveElement\(\)\);/, 'active block selection should sync table alignment from the focused table cell');
assert.match(editorBlocks, /const setTableAlignmentSelectValue = \(alignment, value\) => \{[\s\S]*alignment\.value = normalized;[\s\S]*option\.selected = option\.value === normalized;[\s\S]*const syncActiveTableAlignmentFromEditable = \(activeBlock, editable\) => \{[\s\S]*tablePositionFromCellInput\(cell\)[\s\S]*syncTableAlignmentControlForPosition\(block, normalized\);/, 'table alignment control should force the select value and selected option from active table cell DOM state');
assert.match(editorBlocks, /const isActiveTablePosition = \(block, position\) => \{[\s\S]*state\.activeTableCell[\s\S]*active\.blockId === block\.id[\s\S]*active\.section === position\.section[\s\S]*active\.col === position\.col[\s\S]*const applyIfCurrent = \(\) => \{[\s\S]*isActiveTablePosition\(block, normalized\)[\s\S]*runtime\.requestFrame\(applyIfCurrent\)[\s\S]*runtime\.setTimer\(applyIfCurrent, 0\);/, 'deferred table alignment sync should not overwrite newer active table cells');
assert.doesNotMatch(editorBlocks, /elements\.blocksList/, 'table alignment sync should use the editor list element available in this closure');
assert.doesNotMatch(editorBlocks, /cssEscape/, 'table alignment sync should not depend on unavailable global CSS escape helpers');
