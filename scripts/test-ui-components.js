...
assert.match(themeLayout, /function createThemeLayoutState\(\) \{[\s\S]*layoutMountGeneration: 0[\s\S]*function isCurrentMountGeneration\(generation, options = \{\}\)[\s\S]*if \(!isCurrentMountGeneration\(mountGeneration, options\)\) return null;[\s\S]*applyManifestStyles\(pack, manifest\)[\s\S]*if \(!isCurrentMountGeneration\(mountGeneration, options\)\) return null;[\s\S]*setThemeLayoutContext\(context\)/, 'theme layout reset mounts should ignore stale generations before writing DOM or context state');
