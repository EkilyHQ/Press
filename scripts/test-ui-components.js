...
assert.match(editorPreviewAppRuntime, /createBrowserEditorAppRuntime\(options\)[\s\S]*function postToParent\(payload\)[\s\S]*runtime\.browser\.postMessage[\s\S]*function onRenderMessage\(handler\)[\s\S]*runtime\.events\.onWindow\('message', handler\)[\s\S]*function isTrustedMessageEvent\(event\)[\s\S]*runtime\.browser\.getLocationOrigin\(\)[\s\S]*function applyColorMode\(siteConfig = \{\}\)[\s\S]*runtime\.storage\.getItem\('theme'\)[\s\S]*function applyThemeStyleLinks\([\s\S]*runtime\.globals\.setString\(THEME_PACK_HREF_GLOBAL, primaryHref\)[\s\S]*function querySelector\(selector\)[\s\S]*function getBody\(\)[\s\S]*function getThemeLayoutPackFallback\(\)[\s\S]*function getPreviewStatusElement\([\s\S]*function fetchText\(filename\)/, 'editor preview app runtime should own parent messaging, message-origin checks, color-mode storage/media reads, theme style globals, preview DOM fallbacks, and fetch helpers');
assert.match(editorPreviewRuntime, /const previewRuntime = createEditorPreviewAppRuntime\(\)[\s\S]*function postToParent\(payload\) \{[\s\S]*previewRuntime\.postToParent\(payload\)[\s\S]*function applyPreviewColorMode\(siteConfig = \{\}\) \{[\s\S]*previewRuntime\.applyColorMode\(siteConfig\)[\s\S]*previewRuntime\.applyThemeStyleLinks\(\{[\s\S]*previewRuntime\.querySelector\('\[data-theme-region="main"\], \.native-mainview'\)[\s\S]*previewRuntime\.getThemeLayoutPackFallback\(\)[\s\S]*previewRuntime\.getBody\(\)[\s\S]*previewRuntime\.getPreviewStatusElement\(\)[\s\S]*previewRuntime\.onRenderMessage\(\(event\) => \{[\s\S]*previewRuntime\.isTrustedMessageEvent\(event\)/, 'editor preview runtime should route iframe app effects and DOM fallbacks through its explicit runtime boundary');
