...
assert.match(composerSystemThemeBridge, /from '\.\/system-updates\.js\?v=[\w.-]+';/, 'system/theme bridge should cache-bust system updates after version compatibility changes');
...
assert.match(composerSystemThemeBridge, /from '\.\/theme-manager\.js\?v=[\w.-]+';/, 'system/theme bridge should cache-bust theme manager after Press engine compatibility changes');
assert.match(composer, /from '\.\/encrypted-content\.js\?v=[\w.-]+';/, 'composer should use the encrypted markdown envelope helpers for protected editor flows');
assert.match(themeLayout, /from '\.\/i18n\.js\?v=[\w.-]+';/, 'theme layout should share the repository deletion docs i18n cache key');
assert.match(languageManifest, /en\.js\?v=[\w.-]+/, 'language manifest should cache-bust bundles when editor asset deletion strings change');
assert.doesNotMatch(
...
assert.match(themeLayout, /NATIVE_STYLE_CACHE_KEY = '[\w.-]+'/, 'theme layout should cache-bust manifest-applied native stylesheet changes');
...
assert.match(indexHtml, /src="assets\/js\/theme-boot\.js\?v=[\w.-]+"/, 'index should cache-bust theme boot after external fallback changes');
...
assert.match(indexEditorHtml, /href="assets\/themes\/native\/theme\.css\?v=[\w.-]+" data-editor-native-theme/, 'editor shell should use the fixed native theme stylesheet');
assert.match(indexEditorHtml, /src="assets\/js\/editor-main\.js\?v=[\w.-]+"/, 'editor should cache-bust runtime changes for default blocks view behavior');
...
assert.match(indexEditorPreviewHtml, /<link rel="stylesheet" id="theme-pack">[\s\S]*editor-preview-runtime\.js\?v=[\w.-]+/, 'editor preview iframe should own the theme-pack stylesheet and runtime');
assert.match(editorMain, /from '\.\/editor-main-preview-session\.js\?v=[\w.-]+'/, 'editor main should cache-bust the editor preview session boundary');
...
assert.match(editorPreviewRuntime, /from '\.\/theme-layout\.js\?v=[\w.-]+';/, 'editor preview runtime should cache-bust theme layout generation changes');
assert.match(editorPreviewRuntime, /from '\.\/editor-preview-app-runtime\.js\?v=[\w.-]+';/, 'editor preview runtime should cache-bust its explicit app runtime boundary');
...
assert.doesNotMatch(editorPreviewRuntime, /import \{ getContentRoot, setSafeHtml \} from '\.\/safe-html\.js\?v=[\w.-]+';/, 'editor preview runtime should not import ambient safe-html content-root reads');
assert.match(editorPreviewRuntime, /import \{ setSafeHtml \} from '\.\/safe-html\.js\?v=[\w.-]+';[\s\S]*function getContentRoot\(\) \{[\s\S]*previewRuntime\.getContentRoot\(\)[\s\S]*function applyPreviewContentRoot\(payload = \{\}\) \{[\s\S]*previewRuntime\.setContentRoot\(inferPayloadContentRoot\(payload\)\)[\s\S]*function getImageResolutionOptions\(\) \{[\s\S]*contentRoot: getContentRoot\(\),[\s\S]*origin: previewRuntime\.getLocationOrigin\(\)[\s\S]*mdParse\(markdown, baseDir, \{ imageResolution \}\)[\s\S]*setPreviewSafeHtml\(main, output\.post \|\| '', baseDir, \{ alreadySanitized: true, imageResolution \}\)/, 'editor preview runtime should resolve content root and rendered image paths through the explicit preview app runtime');
...
assert.match(nativeInteractions, /from '\.\.\/\.\.\/\.\.\/js\/theme\.js\?v=[\w.-]+'/, 'native interactions should cache-bust theme helper imports');
assert.match(nativeInteractions, /from '\.\.\/\.\.\/\.\.\/js\/tags\.js\?v=[\w.-]+'/, 'native interactions should cache-bust tag helper imports');
...
assert.match(nativeInteractions, /function loadNativeLinkCardsModule\(runtimeState = null\)[\s\S]*cache\.modulePromise = import\('\.\.\/\.\.\/\.\.\/js\/link-cards\.js\?v=[\w.-]+'\)\.catch\(\(err\) => \{[\s\S]*cache\.modulePromise = null;[\s\S]*throw err;/, 'native lazy link-card fallback should retry after a transient dynamic import failure inside runtime state');
