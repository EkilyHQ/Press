...
assert.match(nativeInteractions, /function createNativeInteractionsRuntimeState\(context = \{\}\) \{[\s\S]*i18n: context && context\.i18n && typeof context\.i18n === 'object' \? context\.i18n : null,[\s\S]*regions: context && context\.regions && typeof context\.regions === 'object' \? context\.regions : null,[\s\S]*hasInitiallyRendered: false,[\s\S]*pendingHighlightRaf: 0,[\s\S]*tabsResizeTimer: 0,[\s\S]*responsiveObserverBound: false,[\s\S]*masonryHandlersBound: false/, 'native interactions should create per-mount runtime state for theme i18n, regions, and transient UI flags');
