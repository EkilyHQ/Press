...
assert.match(nativeInteractions, /function createNativeInteractionsRuntimeState\(context = \{\}\) \{[\s\S]*regions: context && context\.regions && typeof context\.regions === 'object' \? context\.regions : null,[\s\S]*hasInitiallyRendered: false,[\s\S]*pendingHighlightRaf: 0,[\s\S]*tabsResizeTimer: 0,[\s\S]*responsiveObserverBound: false,[\s\S]*masonryHandlersBound: false/, 'native interactions should create per-mount runtime state for theme regions and transient UI flags');
