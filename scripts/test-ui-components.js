...
assert.match(nativeInteractions, /installLightbox\(\{[\s\S]*rootRegion: \['main'\],[\s\S]*document: documentRef,[\s\S]*window: windowRef,[\s\S]*getRegion: \(names\) => getRegion\(names, documentRef\)[\s\S]*\}\)/, 'native interactions should install lightbox through active runtime refs and theme context region reader');
...
assert.match(nativeInteractions, /function createNativeInteractionsRuntimeState\(\) \{[\s\S]*hasInitiallyRendered: false,[\s\S]*pendingHighlightRaf: 0,[\s\S]*tabsResizeTimer: 0,[\s\S]*responsiveObserverBound: false,[\s\S]*masonryHandlersBound: false/, 'native interactions should create per-mount runtime state for transient UI flags');
