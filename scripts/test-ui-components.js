...
assert.match(editorBlocks, /const syncTableAlignmentControlForPosition = \(block, position\) => \{[\s\S]*\.blocks-table-align-select[\s\S]*alignment\.value = normalizeTableAlignment\(table\.alignments\[normalized\.col\]\);[\s\S]*const setActiveTablePosition = \(block, position\) => \{[\s\S]*syncTableAlignmentControlForPosition\(block, normalized\);/, 'table alignment control should refresh when the active table cell changes');
