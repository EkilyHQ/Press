...
assert.match(main, /await ensureThemeLayout\(\{ features: getSiteFeatureContext\(\) \}\);\s*setBootProgress\(0\.6\);/, 'main should advance boot progress after the theme layout mounts');
...
assert.match(editorPreviewRuntime, /const features = createSiteFeatureContext\(payload\.siteConfig \|\| \{\}\)[\s\S]*themeLayout\.ensureThemeLayout\(\{[\s\S]*pack: requestedPack,[\s\S]*persist: false,[\s\S]*reset,[\s\S]*features[\s\S]*\}\)[\s\S]*createRuntimeContext\(\{ payload, containers, content, features \}\)/, 'editor preview runtime should mount themes without persisting themePack and reuse one feature context through its injected layout controller');
