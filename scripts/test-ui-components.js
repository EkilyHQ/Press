...
assert.match(editorPreviewAppRuntime, /createEditorAppRuntime\(options\)[\s\S]*function postToParent\(payload\)[\s\S]*runtime\.browser\.postMessage[\s\S]*function onRenderMessage\(handler\)[\s\S]*runtime\.events\.onWindow\('message', handler\)[\s\S]*function isTrustedMessageEvent\(event\)[\s\S]*runtime\.browser\.getLocationOrigin\(\)[\s\S]*function applyColorMode\(siteConfig = \{\}\)[\s\S]*runtime\.storage\.getItem\('theme'\)[\s\S]*function fetchText\(filename\)/, 'editor preview app runtime should own parent messaging, message-origin checks, color-mode storage/media reads, and fetch helpers');
assert.match(editorPreviewRuntime, /const previewRuntime = createEditorPreviewAppRuntime\(\)[\s\S]*function postToParent\(payload\) \{[\s\S]*previewRuntime\.postToParent\(payload\)[\s\S]*function applyPreviewColorMode\(siteConfig = \{\}\) \{[\s\S]*previewRuntime\.applyColorMode\(siteConfig\)[\s\S]*previewRuntime\.onRenderMessage\(\(event\) => \{[\s\S]*previewRuntime\.isTrustedMessageEvent\(event\)/, 'editor preview runtime should route iframe app effects through its explicit runtime boundary');
assert.doesNotMatch(editorPreviewRuntime, /window\.parent\.postMessage|window\.location\.origin|window\.addEventListener\('message'|window\.matchMedia|localStorage\.getItem\('theme'\)|console\.warn|fetch\(/, 'editor preview runtime should not rediscover parent messaging, origin checks, color-mode storage/media, logging, or fetch globals directly');
