...
| --- | --- | --- |
| 基础单元格 | 支持 | 每一行写在单独一行 |
| 行内 Markdown | 支持 | **粗体**、`代码` 和链接可用 |
| 可视化表格编辑 | 暂不支持 | 块编辑器会将表格保留为 Markdown 源码 |
```
...
`:---`、`:---:`、`---:` 这类对齐标记可以出现在分隔行中并被识别为合法表格，但 Press 目前不会为它们额外设置对齐样式，实际对齐由当前主题决定。表格暂不支持单元格内换行、单元格内转义管道符、`colspan` 或 `rowspan`。
