...
| インライン Markdown | 対応 | **太字**、`コード`、リンクが使えます |
| 視覚的な表編集 | 未対応 | ブロックエディターでは Markdown ソースとして保持されます |
```
...
`:---`、`:---:`、`---:` のような配置マーカーは区切り行で認識されますが、Press は現在それらに追加の配置スタイルを設定しません。実際の配置は有効なテーマに従います。表はセル内の改行、セル内のエスケープされたパイプ、`colspan`、`rowspan` には対応していません。
