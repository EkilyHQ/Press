...
tags:
  - Press
  - 文件
excerpt: Press v3.4.125 的目前文件。使用 Markdown 與基礎 YAML 檔案直接建立內容網站，並說明目前內容索引格式、語言回退、主題、搜尋、SEO、媒體與部署。
author: Ekily
ai: true
---
...
> 你可以從 [v3.4.125/site.yaml](https://github.com/EkilyHQ/Press/blob/v3.4.125/site.yaml) 取得 v3.4.125 的預設設定。
...
在分隔列中使用 `:---`、`:---:`、`---:` 可以分別設定靠左、置中和靠右對齊。省略冒號時使用目前主題的預設對齊方式。Markdown 編輯器的 Blocks 視圖可以視覺化編輯標準 pipe table，包括儲存格、列、欄和欄位對齊。表格暫不支援儲存格內換行、儲存格內轉義管線符、`colspan` 或 `rowspan`；不支援的表格樣式 Markdown 會繼續作為 Markdown 原始碼區塊編輯。
...
- `?lang=cht-tw` — UI 語言偏好，會儲存在 localStorage；內容會先嘗試匹配該語言，再按執行時回退順序選擇可用版本。
...
- 對每篇文章或頁面，Press 會先嘗試載入與目前 UI 語言相同的內容版本。若該版本不存在，v3.4.125 會回退到初始文件/預設語言決定的執行時基礎語言；標準模板通常是 `en`。
- 如果該基礎語言版本也不存在，Press 會繼續嘗試 `en`、`default`，最後使用該項目下第一個可用版本，以避免頁面完全無法渲染。`site.yaml` 中的 `defaultLanguage` 用於沒有明確或已儲存語言選擇時的初始 UI/內容語言；若需要穩定的缺失內容回退，請保留 `en` 或 `default` 版本。

內容索引支援三種形式：

- 簡化版：在基礎 `index.yaml` 與 `tabs.yaml` 中按語言直接提供 Markdown 路徑。
- 統一版：在這些基礎檔案中，為每種語言使用 `{title, location}`。
- 基礎扁平條目：小型網站也可以在基礎檔案中使用 `Guide: post/guide.md` 或 `About: { location: tab/about.md }`。

舊的分語言 sidecar 檔案，例如 `index.en.yaml` 或 `tabs.en.yaml`，現在只是遷移來源，不再是目前執行時的回退路徑。如果網站仍使用這些檔案，請先升級到 Press v3.4.124，開啟編輯器並發布內容模型遷移，再升級到 v3.4.125 或更新版本。
